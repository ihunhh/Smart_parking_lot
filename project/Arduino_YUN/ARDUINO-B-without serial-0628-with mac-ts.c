...
void process(byte id)
{
  ...
  //              //Serial.print(strcat(strcat("LON",strchr(str,'#')),'\n'));
    ...
    else if(strstr(str,"Delete")!=NULL)
    {
      ...
      //Serial.println("");
      del_temp=strstr(str,"Delete")+8;
      if(strstr(del_temp,strcat(PiPID,"1M"))!=NULL)
      {
        digitalWrite(PARK_P1,LOW);
      }
      else if(strstr(del_temp,strcat(PiPID,"2M"))!=NULL)
      {
        digitalWrite(PARK_P2,LOW);
      }
      else if(strstr(del_temp,strcat(PiPID,"3M"))!=NULL)
      {
        digitalWrite(PARK_P3,LOW);
      }
      else if(strstr(del_temp,strcat(PiPID,"4M"))!=NULL)
      {
        digitalWrite(PARK_P4,LOW);
      }
      cur=PARK.END;
      if(strstr(del_temp,PM)!=NULL)
      {
        remv_flag=1;
        digitalWrite(PARK_P1,LOW);
    		digitalWrite(PARK_P2,LOW);
...
void pri_info(LINKLIST_PAR U)
{
  ...
  while(temp)
  {
    ...
    for(i=0;i<=5;i++)
    {
      if(i==5)
      {
        Serial.println(temp->mac[i]);
      }
      else
      {
        Serial.print(temp->mac[i]);
        Serial.print(":");
      }           
    }
    j++;
